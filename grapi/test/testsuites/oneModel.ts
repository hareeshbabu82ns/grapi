...
export function testSuits() {
    ...
    it( 'should create many records to tests where & pagination filter', async () => {
    ...
    ` )
        ...
        // we already created one user in previous tests
    ...
    // it( 'should create many records to tests relay style api', async () => {
    //     // create 20 users, make 10 of them having status NOT_OK
    //     await Promise.all(
    //         times( 20, index => {
    //             const data = index < 10 ? fakeUserData( { status: 'NOT_OK' } ) : fakeUserData();
